...
//        swap(s[0], s[1]);
    ...
    for (int unsorted = 1; unsorted < s.size(); ++unsorted) {
        int stdLocation = unsorted;
        Student nextStudent = s[unsorted];
        
        while ((stdLocation > 0) && comp(nextStudent, s[stdLocation - 1])) {
            s[stdLocation] = s[stdLocation -1];
            stdLocation--;
        }
        
        s[stdLocation] = nextStudent;
...
int main()
{
    ...
    // destruction time.
    {
        ...
        for (int i = 0; i < auxStudents.size(); i++) {
            Student tmpStd = auxStudents[i];
            sPtr.push_back(&tmpStd);
        }
